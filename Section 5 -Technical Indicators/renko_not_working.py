# -*- coding: utf-8 -*-
"""
Created on Sun Mar 27 10:09:34 2022
...
for stock in stocks:
       ...
       ohlc_5min[stock] = temp
       temp = yf.download(stock, period = "1y", interval = "1h")
       temp.dropna(how = "any", inplace =True)
       ohlc_hourly[stock] = temp
...
def renko_DF(DF_min, DF_hour):
    df = DF_min.copy()
    df.reset_index(inplace =True)
    df.drop("Close",axis =1,inplace =True)
    df.columns = ["date","open","high","low","close","volume"]
    df2 =Renko(df)
    df2.brick_size = 3*round(ATR(ohlc_hourly[stock],120).iloc[-1],0)
    renko_df = df2.get_ohlc_data()
    return renko_df
    

for stock in stocks:
    renko_data[stock] = renko_DF(ohlc_5min[stock],ohlc_hourly[stock])
